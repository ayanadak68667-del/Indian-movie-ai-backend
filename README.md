// server.js (Hostinger Horizons ফিক্সড ভার্সন)

const express = require('express');
const axios = require('axios');
const cors = require('cors');
require('dotenv').config();

const app = express();
app.use(cors());
app.use(express.json());

// ENV keys
const TMDB_KEY = process.env.TMDB_API_KEY;
const GEMINI_KEY = process.env.GEMINI_API_KEY;

const TMDB_BASE = 'https://api.themoviedb.org/3';
const GEMINI_URL = `https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent`;

// ---------- Helper: call TMDB discover for a Movie section ----------
async function fetchMovieSection(sortBy, languageCode) {
  const url = `${TMDB_BASE}/discover/movie`;
  const params = {
    api_key: TMDB_KEY,
    language: 'en-US',   // লেবেল ইংরেজি, কিন্তু কনটেন্ট সব ভাষার
    region: 'IN',
    sort_by: sortBy,     // popularity.desc / release_date.asc / vote_average.desc
    'vote_count.gte': 50,
    page: 1
  };

  if (languageCode && languageCode !== 'all') {
    params.with_original_language = languageCode;
  }

  const res = await axios.get(url, { params });
  return res.data.results.slice(0, 12);
}

// ---------- Helper: call TMDB discover for a TV/Series section (NEW) ----------
async function fetchSeriesSection(sortBy) {
    const url = `${TMDB_BASE}/discover/tv`;
    const params = {
        api_key: TMDB_KEY,
        language: 'en-US',
        region: 'IN',
        sort_by: sortBy,
        'vote_count.gte': 50,
        page: 1
    };
    const res = await axios.get(url, { params });
    return res.data.results.slice(0, 12);
}

// ---------- Home sections (Horizons-এর প্রত্যাশিত পাথে পরিবর্তন) ----------

// 1. Trending Movies (Horizons Path: /trending-movies)
app.get('/trending-movies', async (req, res) => {
  try {
    const lang = req.query.language || 'all';
    const results = await fetchMovieSection('popularity.desc', lang);
    res.json({ results });
  } catch (e) {
    console.error(e.response?.data || e.message);
    res.status(500).json({ error: 'Failed to load trending movies' });
  }
});

// 2. Upcoming Movies (Horizons Path: /upcoming-movies)
app.get('/upcoming-movies', async (req, res) => {
  try {
    const lang = req.query.language || 'all';
    const results = await fetchMovieSection('release_date.asc', lang);
    res.json({ results });
  } catch (e) {
    console.error(e.response?.data || e.message);
    res.status(500).json({ error: 'Failed to load upcoming movies' });
  }
});

// 3. Top Rated Movies (Horizons Path: /top-rated-movies)
app.get('/top-rated-movies', async (req, res) => {
  try {
    const lang = req.query.language || 'all';
    const results = await fetchMovieSection('vote_average.desc', lang);
    res.json({ results });
  } catch (e) {
    console.error(e.response?.data || e.message);
    res.status(500).json({ error: 'Failed to load top rated movies' });
  }
});

// 4. Popular Web Series (NEW - Horizons Path: /popular-web-series)
app.get('/popular-web-series', async (req, res) => {
    try {
        // ওয়েব সিরিজের জন্য জনপ্রিয়তা অনুযায়ী সর্ট করা হয়েছে
        const results = await fetchSeriesSection('popularity.desc');
        res.json({ results });
    } catch (e) {
        console.error(e.response?.data || e.message);
        res.status(500).json({ error: 'Failed to load popular web series' });
    }
});


// ---------- Search endpoint (পুরোনো রুট বজায় রাখা হলো) ----------

app.get('/api/search', async (req, res) => {
  try {
    const query = req.query.query || '';
    const lang = req.query.language || 'all';

    if (!query.trim()) {
      return res.json({ results: [] });
    }

    const params = {
      api_key: TMDB_KEY,
      query,
      language: 'en-US',
      region: 'IN',
      include_adult: false,
      page: 1
    };

    if (lang && lang !== 'all') {
      params.with_original_language = lang;
    }

    const tmdbRes = await axios.get(`${TMDB_BASE}/search/movie`, { params });
    res.json({ results: tmdbRes.data.results || [] });
  } catch (e) {
    console.error(e.response?.data || e.message);
    res.status(500).json({ error: 'Search failed' });
  }
});

// ---------- Movie detail + AI blog + OTT + trailer (পুরোনো রুট বজায় রাখা হলো) ----------

app.get('/api/movie/:id', async (req, res) => {
  try {
    const id = req.params.id;

    const detailsRes = await axios.get(`${TMDB_BASE}/movie/${id}`, {
      params: {
        api_key: TMDB_KEY,
        append_to_response: 'videos,watch/providers,credits,external_ids'
      }
    });

    const movie = detailsRes.data;

    const trailer = (movie.videos?.results || []).find(
      (v) => v.type === 'Trailer' && v.site === 'YouTube'
    );
    const trailerUrl = trailer
      ? `https://www.youtube.com/embed/${trailer.key}`
      : null;

    const providersIN = movie['watch/providers']?.results?.IN || {};
    const ottLinks = (providersIN.flatrate || [])
      .filter((p) =>
        ['Netflix', 'Amazon Prime Video'].includes(p.provider_name)
      )
      .map((p) => ({
        name: p.provider_name,
        url: `https://www.google.com/search?q=${encodeURIComponent(
          `${movie.title} ${p.provider_name} watch online`
        )}`
      }));

    const mainCast = (movie.credits?.cast || [])
      .slice(0, 5)
      .map((c) => c.name)
      .join(', ');
    const director =
      (movie.credits?.crew || []).find((c) => c.job === 'Director')?.name ||
      'Unknown';

    const genres = (movie.genres || []).map((g) => g.name);
    const langCode = movie.original_language || 'hi';

    const prompt = `
You are an Indian movie critic. Write a short English blog-style analysis (100-150 words)
for the movie "${movie.title}".
Release date: ${movie.release_date}.
Director: ${director}.
Main cast: ${mainCast}.
Genres: ${genres.join(', ')}.
Language: ${langCode}.
Focus on storyline, performances and box-office potential in a dark, cinematic tone.
`;

    let aiBlog = '';
    if (GEMINI_KEY) {
      const aiRes = await axios.post(
        `${GEMINI_URL}?key=${GEMINI_KEY}`,
        { contents: [{ parts: [{ text: prompt }] }] }
      );
      aiBlog =
        aiRes.data.candidates?.[0]?.content?.parts?.[0]?.text || '';
    }

    res.json({
      id: movie.id,
      title: movie.title,
      overview: movie.overview,
      poster_path: movie.poster_path,
      backdrop_path: movie.backdrop_path,
      vote_average: movie.vote_average,
      release_date: movie.release_date,
      genres,
      language: langCode,
      imdb_id: movie.imdb_id || null,
      imdb_url: movie.imdb_id
        ? `https://www.imdb.com/title/${movie.imdb_id}`
        : null,
      trailer_url: trailerUrl,
      ott_links: ottLinks,
      ai_blog: aiBlog
    });
  } catch (e) {
    console.error(e.response?.data || e.message);
    res.status(500).json({ error: 'Failed to load movie details' });
  }
});

// ---------- "You May Also Like" recommendations (পুরোনো রুট বজায় রাখা হলো) ----------

app.get('/api/movie/:id/recommendations', async (req, res) => {
  try {
    const id = req.params.id;

    const simRes = await axios.get(`${TMDB_BASE}/movie/${id}/similar`, {
      params: {
        api_key: TMDB_KEY,
        language: 'en-US',
        region: 'IN',
        page: 1
      }
    });

    let list = (simRes.data.results || []).slice(0, 20);
    list = list.slice(0, 8);

    res.json({ results: list });
  } catch (e) {
    console.error(e.response?.data || e.message);
    res.status(500).json({ error: 'Failed to load recommendations' });
  }
});

// ---------- 24/7 AI Chatbot (পুরোনো রুট বজায় রাখা হলো) ----------

app.post('/api/chat', async (req, res) => {
  try {
    const question = req.body.message || '';

    if (!GEMINI_KEY) {
      return res.json({ reply: 'Gemini API key not set.' });
    }

    const systemPrompt = `You are an Indian movie assistant.
Answer in English.
You know movies and series from all Indian languages (Hindi, Tamil, Telugu,
Malayalam, Kannada, Marathi, Bengali, and others).`;

    const aiRes = await axios.post(
      `${GEMINI_URL}?key=${GEMINI_KEY}`,
      {
        contents: [
          { parts: [{ text: systemPrompt }] },
          { parts: [{ text: question }] }
        ]
      }
    );

    const reply =
      aiRes.data.candidates?.[0]?.content?.parts?.[0]?.text || 'No reply.';
    res.json({ reply });
  } catch (e) {
    console.error(e.response?.data || e.message);
    res.status(500).json({ error: 'Chat failed' });
  }
});

// ---------- Health check (রুট বজায় রাখা হলো) ----------
// এটি Render চেক করে সার্ভার চলছে কিনা
app.get('/health', (req, res) => res.send('OK'));

// এটি 'Cannot GET /' সমস্যা এড়াতে একটি ডিফল্ট রুট
app.get('/', (req, res) => res.send('Backend Server is Live!')); 


// ---------- Start server ----------

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`Server running on ${PORT}`));
